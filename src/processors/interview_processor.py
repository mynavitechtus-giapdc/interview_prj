...
class InterviewProcessor:
    """Main processor for interview system"""
    ...
    def _search_question_in_vectorstore(self, question_text: str) -> Tuple[Optional[dict], Optional[float]]:
        """
        Search for similar question in vector store
    ...
    def _re_summarize_question(self, question: str) -> str:
        """Re-summarize question using LLM"""
        try:
    ...
    def process_answer(
        ...
        """
        Process candidate's answer following the correct flow
        
        Flow:
        1. Search question_summarized in vector store
           ├─ FOUND → Check if answer exists in DB
           │          ├─ YES → Grade with DB answer
           │          └─ NO → Generate answer with LLM → Grade
           │
           └─ NOT FOUND → Re-summarize with LLM
